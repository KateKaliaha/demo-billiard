...
export const useBilliardLogic = (
  ...
  const [selectedBallForColorChange, setSelectedBallForColorChange] = useState<
    number | null
  >(null);
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (cueStickPosition.current.end === null) {
      if (selectedBallForColorChange) {
        ...
        setSelectedBallForColorChange(clickedIndex.current);
        cueStickPosition.current = { start: null, end: null };
      }
    }
    ...
    clickedIndex.current = null;
    cueStickPosition.current = { start: null, end: null };
  }, [ballsRef, selectedBallForColorChange]);
